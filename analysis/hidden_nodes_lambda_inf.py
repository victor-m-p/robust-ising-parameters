...
# just test a couple 
groundtruth = np.loadtxt('../data/hidden_nodes/Jh_questions_8_samples_500_scale_0.5.dat')
...
d_groundtruth_Jij = pd.DataFrame({
    'Jij': Jij_ind,
    'grountruth': groundtruthJ
})
#d_Jij_gt = d_groundtruth_Jij.merge(d_Jij_subset, on='Jij', how='inner')
#sns.scatterplot(x='grountruth', y='coupling', hue='id', data=d_Jij_gt)
...
sns.scatterplot(x='grountruth', y='coupling', hue='type', data=d_Jij_gt)
...
for i in range(8): 
    ...
    dns = dns[dns['id'] == f'{i}']
    sns.scatterplot(x='grountruth', y='coupling', hue='type', data=dns)
    plt.plot([-1.5, 1.5], [-1.5, 1.5], 'k-')
