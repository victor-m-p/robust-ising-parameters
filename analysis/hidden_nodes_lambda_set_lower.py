...
# just test a couple 
groundtruth = np.loadtxt('../data/hidden_nodes_0.25_logs0/Jh_questions_8_samples_500_scale_0.25.dat')

# load hidden nodes data dynamical lambda 
files = os.listdir('../data/hidden_nodes_0.25_logs0/')
...
regex1 = r'questions_8_samples_500_scale_0.25_hidden_(\w+)_(\w+)'
regex2 = r'questions_8_samples_500_scale_0.25_hidden_(\w+)_(\w+)'
tst = files_hidden[1]
if re.search(regex1, tst): 
    hidden1, hidden2 = re.search(regex1, tst).group(1, 2)
    
    
else: 
    hidden = re.search(regex2, tst).group(1)

tst = files_hidden[2]
matches = re.search(regex, tst)
matches.group(1)

## first: still only 1 hidden node ## 
# focus on Jij for now 
Jij_list = []
hi_list = []
for file in files_hidden:
    params = np.loadtxt('../data/hidden_nodes_logs0/' + file)
    idx = re.search(regex, file).group(1)
    n_params = len(params)
